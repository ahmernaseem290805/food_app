...
export default function OrdersPage(){
 ...
 const load=async()=>{const [s,r]=await Promise.all([getDocs(collection(db,"Orders")),getDocs(collection(db,"Riders"))]);setRiders(r.docs.map(d=>({id:d.id,...d.data()})));setOrders(s.docs.map(d=>({id:d.id,...d.data()})).sort((a:any,b:any)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)));setLoading(false)};useEffect(()=>{load()},[]);
