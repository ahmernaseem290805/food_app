...
export default function NotificationsPage(){
 ...
 const load=async()=>{const s=await getDocs(collection(db,"Notifications"));setItems(s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)))};
 useEffect(()=>{load()},[]);
